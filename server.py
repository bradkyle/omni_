...
class Envs(object):
    ...
    config sets the initial config of the agent in the
    environment and can be changed by the agent (todo implement)
    """
    def __init__(self):
        self.envs = {}
        self.id_len = 8
    ...
    # would simply remove an instance of a channel
    ...
    def create(self, env_id, seed=None, config=None):
        try:
            env = gym.make(env_id)
            if seed:
                env.seed(seed)
...
@app.route('/v1/envs/', methods=['POST']) #todo add channel config
def env_create():
    """
